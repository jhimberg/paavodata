...
map_fi_postinumero <- 
  function(df, title.label = NA, viridis.option = "E", color.limits = c(NA,NA)) {
    ...
    # title.label: string 
    # viridis.option: viridis colormap, default "E" (cividis)
    # color.limits: c(min, max) set min and max values in data mapped to colorscale 
    #  (default: set to data min-max c(NA,NA). Values ouside the range get missing value color
...
paavo_diff <- function(paavo.df, years = c(2018, 2015)) {
  ...
  diff.attributes <- setdiff(paavo$vars$koodi, c("nimi", "pono", "vuosi", "euref_x", "euref_y", "kuntano")) 
  
  paavo <- paavo.df %>% 
    group_by(pono, pono.level) %>% 
    arrange(vuosi) %>% 
    filter(vuosi %in% years) %>%
    mutate_at(vars(diff.attributes), funs(. - lag(.))) %>% 
